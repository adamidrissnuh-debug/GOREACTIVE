...
import SprintTutorialScene from './SprintTutorialScene';
import { Zap, ChevronRight } from 'lucide-react';
import { GuidedHeader } from '../components/ui/GuidedHeader';
import type { AppCtx } from '../app/useAppCtx';

export function SprintTutorialStep({ ctx }: { ctx: AppCtx }) {
  const {
    appLanguage, setGuidedStep, tutorialStep, setTutorialStep, boxingTutorialMessages,
    currentTutorialMessage, getGuidedProgress,
  } = ctx;

  return (
    <motion.div
      key="guided-sprint-tutorial"
      ...
      <div className="h-[68%] relative overflow-hidden">
        <div className="absolute top-0 inset-x-0 z-40 px-4">
          <GuidedHeader
            ...
            onSkip={() => setGuidedStep('SPRINT_TIME_CHOICE')}
          />
        </div>
        <SprintTutorialScene 
            ...
            <button 
              onClick={() => {
                if (tutorialStep < boxingTutorialMessages.length - 1) {
                  ...
                  setGuidedStep('SPRINT_TIME_CHOICE');
                }
              }}
